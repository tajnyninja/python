...
# słowniczek
cyfry_slownik = {'1': 'one', '2': 'two', '3': 'three', '4': 'four', '5': 'five', '6': 'six', '7': 'seven', '8': 'eight', '9': 'nine'}
...
for linia in lista_linii:
    ...
    # Reset flagi
    flaga = False            
    for i, znak in enumerate(linia):
        string = linia[i:]
        for klucz, wartosc in cyfry_slownik.items():
            if klucz == znak:
                tmp.append(klucz)
                break
            if string.startswith(wartosc):
                tmp.append(klucz)
                break
    first = ''.join(tmp[0])
    last = ''.join(tmp[-1])
