inputvar='''Game 1: 13 green, 3 red; 4 red, 9 green, 4 blue; 9 green, 10 red, 2 blue
...
for linia in lista_linii:
    ...
    #print(len(ss))
    for x in ss:
        # h to konkretny zestaw gry
        h = x.split(',')

        # zerujemy w danym zestawie
        
        #print(h)
        for n in h:
            # detal to posczególne kostki :)           
            detal = n.split()
            #print(n)
            if 'red' == detal[1]:
                if int(detal[0]) > redmax:
                    redmax = int(detal[0])
            if 'green' == detal[1]:
                if int(detal[0]) > greenmax:
                    greenmax = int(detal[0])
            if 'blue' == detal[1]:
                if int(detal[0]) > bluemax:
                    bluemax = int(detal[0])
            
        
            # too high
    suma += redmax * greenmax * bluemax
    print(f"ID: {gameID} - red: {redmax} green: {greenmax} blue: {bluemax}")
