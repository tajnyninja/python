...
# podział na linie
#inputLines = inputvar.splitlines()
...
# długość jednej lini (grid jest kwadratem)
lineLenght = len(grid[0])

# Suma numerów seryjnych
sum = 0

# Pierwsza iteracja po liniach - ↓
for i in range(len(grid)):
    ...
    while j < lineLenght:
        if grid[i][j].isdigit():
            ...
            # start - określa początek liczby
            # dzięki temu że jest jo pętla while
            # to nie ma problemu jak z for i range...
            start = j
            while j < lineLenght and grid[i][j].isdigit():
                j += 1
            number = int(''.join(grid[i][start:j]))

            # Sprawdzanie czy to part number
            is_part_number = False
            for k in range (start, j):
                if checkNeighbors(grid, i, k):
                    is_part_number = True
                    break

            if is_part_number:
                sum += number
        # Jeśli nie jest cyfrą to inkrementuje :)
        else:
            j += 1
